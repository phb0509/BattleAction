...
void DetailCustomization_SetFSMServiceNode::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	check(m_ServiceNode != nullptr);
	
	//const UEnum* enumType = FindObject<UEnum>(ANY_PACKAGE, *m_ServiceNode->GetEnumClassName().ToString(), true); 
	const UEnum* enumType = FindFirstObjectSafe<UEnum>(*m_ServiceNode->GetEnumClassName().ToString()); // 마지막으로 디스크에 저장한 EnumClassName을 Find.
	
	UpdateComboBox(enumType); // nullptr 허용
...
void DetailCustomization_SetFSMServiceNode::OnEnumClassNameCommitted(const FText& NewText, ETextCommit::Type CommitInfo) // 텍스트커밋 발생했을 때 호출함수
{
	...
	check(m_ServiceNode.IsValid());
	
	//const UEnum* enumType = FindObject<UEnum>(ANY_PACKAGE, *NewText.ToString(), true);
	const UEnum* enumType = FindFirstObjectSafe<UEnum>( *NewText.ToString()); // 마지막으로 디스크에 저장한 EnumClassName을 Find.
	
	UpdateComboBox(enumType);

	m_ServiceNode->Modify();
...
{
	// 서비스노드를 클릭 후, 콤보박스를 눌렀을 때 원소의 개수만큼 호출.
...
void DetailCustomization_SetFSMServiceNode::UpdateComboBox(const UEnum* enumType)
{
	...
	if (enumType != nullptr)
	{
		if (enumType->NumEnums() == 0) // EnumClass 정의'만' 돼었을 경우 (원소가 0개)
		{
...
{
	// 콤보박스의 원소들을 선택했을 때 호출. 현재 선택되어져 있는 것을 선택했을 경우엔 호출 안된다.
