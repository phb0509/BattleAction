...
void UMonsterHPBar::NativeConstruct()
{
	...
	{
		UMaterialInterface* materialInterface = Cast<UMaterialInterface>(m_HPProgressBar->GetWidgetStyle().FillImage.GetResourceObject());
		...
		FProgressBarStyle progressBarStyle = m_HPProgressBar->GetWidgetStyle();
		...
		//m_HPProgressBar->GetWidgetStyle().FillImage.SetResourceObject(m_HPProgressBarMaterialInstanceDynamic.Get()); // 구버전
	}
	...
	FSlateBrush brush = m_HPBarBorder->GetBrush(); 
...
void UMonsterHPBar::OnRecoveredHPBar() // 틱마다 호출.
{
	...
	if (m_HPProgressBar->GetPercent() >= m_GuardRegainProgressBar->GetPercent())
	{
		m_GuardRegainProgressBar->SetPercent(m_HPProgressBar->GetPercent());
	}
}

void UMonsterHPBar::updateGuardRegainProgressBar()
{
	float updatedPercent = m_GuardRegainProgressBar->GetPercent() - m_DecreaseSpeed;

	if (updatedPercent <= m_HPProgressBar->GetPercent())
	{
		m_GuardRegainProgressBar->SetPercent(m_HPProgressBar->GetPercent());
